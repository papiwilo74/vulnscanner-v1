...
def test_record_and_read_feedback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_train_ai_active_learning_loader(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    test_feedback_file = str(tmp_path / "ai_feedback.jsonl")
    ...
    monkeypatch.setattr(os.path, "join", lambda *args: test_feedback_file if "ai_feedback.jsonl" in args else str(tmp_path))
